...
$query = "(SELECT id, NULL AS location_order_id, product, milestone, capability_standard, flow_type, created, 'order' as type FROM test_orders WHERE created > '{$expiry}' AND user_id = {$_SESSION['user']->id})
          UNION ALL
          (SELECT id, location_order_id, product, milestone, NULL AS capability_standard, NULL AS flow_type, created, 'ppv_order' as type FROM test_ppv_orders WHERE created > '{$expiry}' AND user_id = {$_SESSION['user']->id}) 
...
if(!empty($orders)) {
    ...
    $query = "(SELECT id, NULL AS location_order_id, product, milestone, capability_standard, flow_type, created, 'order' as type FROM test_orders WHERE created > '{$expiry}' AND user_id = {$_SESSION['user']->id})
              UNION ALL
              (SELECT id, location_order_id, product, milestone, NULL AS capability_standard, NULL AS flow_type, created, 'ppv_order' as type FROM test_ppv_orders WHERE created > '{$expiry}' AND user_id = {$_SESSION['user']->id} AND location_order_id = 0) 
    ...
    <div class="row">
        <div class="col-lg-12 ">
            <div class="main-heading  w-100">
                <h1 class="f-14 my-4">Simulation Testing Result</h1>
            </div>
...
//                        $validation = findByQuery("SELECT * FROM validations WHERE order_id = {$order->id}");
                        ...
                        $ppv_exists = findByQuery("SELECT id FROM test_ppv_orders WHERE location_order_id = {$order->id}")
                        ?>
...
<!--                                --><?php //endif; ?>
                                ...
                                <a href="test-output<?php echo !empty($ppv_exists) ? '-ppv' : '' ?>.php?id=<?php echo !empty($ppv_exists) ? $ppv_exists->id : $order->id ?>"><button class="card-btn text-white btn-fill-2 w-100 mx-0 px-0">Detail</button></a>
                            </div>
                        </div>
                    <?php else:
                        $milestone = findByQuery("SELECT * FROM milestones WHERE id = {$order->milestone}");
...
<!--                                <h1 class="f-20 w-500">--><?php //echo $pmfs->name ?><!--</h1>-->
                                <a href="test-output-ppv.php?id=<?php echo $order->id ?>"><button class="card-btn text-white btn-fill-2 w-100 mx-0 px-0">Detail</button></a>
                            </div>
                        </div>
                    <?php endif; ?>
                <?php endforeach; ?>
...
<!--                                --><?php //endif; ?>
                                ...
                                <p class="f-12 w-400 mb-0 pb-0">Manufacturing Flow STD</p>
                                <h1 class="f-20 w-500"><?php echo $flow_type->name ?></h1>
                                <a href="test-output.php?id=<?php echo $order->id ?>"><button class="card-btn text-white btn-fill-2 w-100 mx-0 px-0">Detail</button></a>
                            </div>
                        </div>
                    <?php endforeach; ?>
                    <!--                --><?php //else: ?>
