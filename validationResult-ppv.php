...
if(isset($_GET['id'])) {
    $query = "SELECT * FROM ppv_orders WHERE id = {$_GET['id']} AND user_id = {$_SESSION['user']->id}";
...
<script>
    ...
    $('.validation').click(function () {
        if(!$(this).hasClass('bg-success')) {
            ...
            $.ajax({
                ...
                data: {table: "validations_ppv", 'validationFiles': true, order_id},
    ...
    $('#validationModal-add').click(function () {
        ...
        if(wdvt == "" || wdvi == "") {
            ...
            formData.append("table", "validations_ppv")
            var that = $(this)
            $.ajax({
                method: 'post',
                url: 'includes/ajax.php',
    ...
    $('.hvm_dep').click(function () {
        if(!$(this).hasClass('bg-success')) {
            ...
            $.ajax({
                method: 'post',
                url: 'includes/ajax.php',
                data: {table: "validations_ppv", 'cbvalidationFiles': true, order_id},
                success: function (response) {
                    console.log(response)
                    var data = JSON.parse(response);
                    ...
                    $(cbt).attr("href", 'uploads/' + data[0].workstream_text)
                    $(cbt).text('uploads/' + data[0].workstream_text)
                    cbi.src = 'uploads/' + data[0].workstream_img
                }
            })
        }
    })
...
<!--        form_data.append('validation-ppv', col);-->
