    ...
    if(isset($_POST['ptp']) && $_POST['ptp'] == 1) {
        if(isset($_POST['capability'])) {
            ...
            if($res) {
                ...
                $query = "INSERT INTO test_ppv_orders (location_order_id, user_id, product, milestone, module, created) VALUES (?,?,?,?,?,?)";
                $stmt = $conn->prepare($query);
                ...
                foreach ($ppv_flow as $key => $value) {
                    $query = "INSERT INTO test_ppv_orders_data (ppv_order_id, ppv_manufacturing_flow_std_id, ppv_id) VALUES (?,?,?)";
                    $stmt = $conn->prepare($query);
                    ...
                    foreach ($ppv_types[$key+1] as $ppv_type) {
                        $query = "INSERT INTO test_ppv_types_orders (ppv_orders_data_id, ppv_id) VALUES (?,?)";
                        $stmt = $conn->prepare($query);
...
//                $res = $stmt->execute([$id]);
                ...
                redirect("test-output-ppv.php?id={$ppv_order_id}");
            } else {
...
//            $res = $stmt->execute([$id]);
            ...
            redirect("test-output.php?id={$id}");
        } else {
            $message = "<p class='alert alert-danger'>Data insertion failed!</p>";
        }
    }
