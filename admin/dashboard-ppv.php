...
if(isset($_GET['id'])) {
    $query = "DELETE FROM ppv_orders WHERE id = ?";
...
$query = "SELECT * FROM ppv_orders WHERE location_order_id = 0";
...
if(!empty($orders)) {
    ...
    $query = "SELECT * FROM ppv_orders WHERE location_order_id = 0 LIMIT {$page}{$perPage}";
    $orders = findAllByQuery($query);
}
    ...
    <div class="row">
        <div class="col-lg-12 ">
            <div class="main-heading  w-100">
                <h1 class="f-14 my-4">Live Dashboard</h1>
            </div>
                ...
                ?>
                    <div class="col-xl-3 col-lg-4 col-md-4 mb-3">
                        <div class="card  d-flex flex-column ">
                            ...
                            <h1 class="f-20 w-500 "><?php echo "PPV Platform" ?></h1>
                            ...
                            <a href="liveOutput-ppv.php?id=<?php echo $order->id ?>"><button class="card-btn text-white btn-fill-2 w-100 mx-0 px-0">Detail</button></a>
                        </div>
                    </div>
                <?php endforeach; ?>
                <?php else: ?>
