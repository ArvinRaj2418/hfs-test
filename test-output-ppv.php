...
$query = "SELECT *, po.id AS poid, p.id as ppvid, pod.ppv_id as podppvid, m.name AS mName, mo.name AS moName FROM test_ppv_orders po INNER JOIN test_ppv_orders_data pod on po.id = pod.ppv_order_id ";
...
$query = "SELECT *, pmfs.id as pmfsid, pod.id as podid, ppvs.id as ppvsid FROM test_ppv_orders_data pod";
...
// $ppvid = explode(",", $data->ppvid);
// echo $data->podppvid;

$query = "SELECT * FROM ppv_data INNER JOIN columns c ON c.id = ppv_data.column_id INNER JOIN ppvs ON ppv_data.ppv_id = ppvs.id WHERE ppv_id IN ({$data->podppvid})";
$locations = findAllByQuery($query);

$locations2 = array();
if($data->location_order_id != 0) {
    $query = "SELECT *,o.id as oID, cs.name AS csName, m.name AS mName, hfs.name AS hfsName, bi.name AS biName, ft.name AS ftName FROM test_orders o ";
                    ...
                    <div class="row">
                        ...
                        <?php foreach ($ppv_data as $key => $ppv_datum): ?>
                            ...
                            <?php
                                ...
                                $query = "SELECT * FROM ppv_data INNER JOIN columns c ON c.id = ppv_data.column_id INNER JOIN ppvs ON ppv_data.ppv_id = ppvs.id WHERE ppv_id IN ({$ppv_datum->ppvsid})";
                                $locations = findAllByQuery($query);

                                $query = "SELECT * FROM test_ppv_types_orders INNER JOIN ppvs ON test_ppv_types_orders.ppv_id = ppvs.id WHERE ppv_orders_data_id = {$ppv_datum->podid}";
                        ...
                        <table id="export-table" class="d-none">
                            <thead>
                                ...
                                <?php endif; ?>
                                    ...
                                    <?php if(!empty($ppv_data)): ?>
                                    <?php foreach ($ppv_data as $key => $ppv_datum): ?>
                                            <th><b>PPV Manufacturing Flow STD Insertion (<?php echo $key + 1 ?>)</b></th>
                                    <th><b>Platform</b></th>
                                        <?php
                                            $query = "SELECT * FROM ppv_data INNER JOIN columns c ON c.id = ppv_data.column_id INNER JOIN ppvs ON ppv_data.ppv_id = ppvs.id WHERE ppv_id IN ({$ppv_datum->ppvsid})";
                                            $locations = findAllByQuery($query);
                                            ...
                                            <?php if(!empty($types)): ?>
                                                <?php foreach ($types as $key2 => $type): ?>
                                                    <th><b>PPV Type <?php echo $key2 + 1 ?></b> </th>
                                                <?php endforeach; ?>
                                            <?php endif; ?>
                                            <?php if(!empty($locations)): ?>
                                                <?php foreach($locations as $location): ?>
                            ...
                            <tbody>
                                ...
                                <?php if($data->location_order_id != 0): ?>
                                    <td><?php echo $dataPost->csname ?></td>
                                    <td><?php echo $dataPost->hfsname ?></td>
                                <?php endif; ?>
                                <?php if(!empty($locations2)): ?>
                                <?php foreach($locations2 as $location): ?>
                                    <td>
                                ...
                                <?php endif; ?>
                                    <?php if(!empty($locations2)): ?>
                                    <td><p><?php echo SITE_URL ?>uploads/<?php echo !empty($locations2) ? $locations2[0]->img : '' ?></p></td>
                                    <?php endif; ?>

                                    <?php if(!empty($ppv_data)): ?>
                                        <?php foreach ($ppv_data as $key => $ppv_datum): ?>
                                            <td><?php echo $ppv_datum->name ?></td>
                                            <td><?php echo $ppv_datum->platform_name ?></td>
                                            <?php
                                            $query = "SELECT * FROM ppv_data INNER JOIN columns c ON c.id = ppv_data.column_id INNER JOIN ppvs ON ppv_data.ppv_id = ppvs.id WHERE ppv_id IN ({$ppv_datum->ppvsid})";
                                            $locations = findAllByQuery($query);
                                            $query = "SELECT * FROM test_ppv_types_orders INNER JOIN ppvs ON test_ppv_types_orders.ppv_id = ppvs.id WHERE ppv_orders_data_id = {$ppv_datum->podid}";
                                            ...
                                            <?php if(!empty($types)): ?>
                                                <?php foreach ($types as $key2 => $type): ?>
                                                    <th>PPV Type <?php echo $key2 + 1 ?> </th>
                                                <?php endforeach; ?>
                                            <?php endif; ?>
                                            <?php if(!empty($locations)): ?>
                                                <?php foreach($locations as $location): ?>
                                            ...
                                            <?php endif; ?>
                                            <?php if(!empty($locations)): ?>
                                                <td><p><?php echo SITE_URL ?>uploads/<?php echo !empty($locations) ? $location->img : '' ?></p></td>
                                            <?php endif; ?>
                                        <?php endforeach; ?>
                                    <?php endif; ?>
                                </tr>
