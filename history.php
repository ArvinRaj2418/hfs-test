...
$expiry = date('Y-m-d', strtotime(date('Y-m-d') . '-90 days'));
...
if(!empty($orders)) {
    ...
    $query = "SELECT * FROM orders WHERE created <= '{$expiry}' AND user_id = {$_SESSION['user']->id} LIMIT {$page}{$perPage}";
    $orders = findAllByQuery($query);
}
    ...
    <div class="row">
        <div class="col-lg-12 ">
            <div class="main-heading  w-100">
                <h1 class="f-14 my-4">Flow History</h1>
            </div>
